...
const CATEGORY_SLUG = 'home-inspector'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `${service.shortDescription} IAC2 certified inspector serving all of Orange County. Call 949-371-5934.`
  return {
    title,
    description,
...
export default async function HomeInspectorServicePage({ params }: Props) {
  const { slug } = await params
  const service = findServiceBySlug(slug)
  ...
  return (
    <main className="pt-28">
      ...
      <section className="py-12 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Other Home Inspector Services</h2>
